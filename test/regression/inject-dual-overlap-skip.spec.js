...
test('dual-overlap-skip: 同一視覺位置的 sibling clone 應 skip，避免 Gmail 類頁面疊譯文', async ({
  context,
  localServer,
}) => {
  const page = await context.newPage();
  await page.goto(`${localServer.baseUrl}/dual.html`, { waitUntil: 'domcontentloaded' });
  await page.waitForSelector('#visual-dup-a', { timeout: 10_000 });

  const { evaluate } = await getShinkansenEvaluator(page);

  await evaluate(`(() => {
    window.__shinkansen.testInjectDual(document.querySelector('#visual-dup-a'), '親愛的客戶');
    window.__shinkansen.testInjectDual(document.querySelector('#visual-dup-b'), '親愛的客戶');
  })()`);

  const result = await page.evaluate(() => ({
    wrapperCount: document.querySelectorAll('shinkansen-translation').length,
    firstHasAttr: document.querySelector('#visual-dup-a').hasAttribute('data-shinkansen-dual-source'),
    secondHasAttr: document.querySelector('#visual-dup-b').hasAttribute('data-shinkansen-dual-source'),
    wrapperText: document.querySelector('shinkansen-translation')?.textContent?.trim(),
  }));

  expect(result.wrapperCount, '同位置 sibling clone 不應插出第二個 wrapper').toBe(1);
  expect(result.firstHasAttr).toBe(true);
  expect(result.secondHasAttr, '第二個 clone 應 skip，不打 dual-source').toBe(false);
  expect(result.wrapperText).toBe('親愛的客戶');
