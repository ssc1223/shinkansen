...
test('youtube-existing-chinese-captions: 啟動時畫面已有中文字幕則不顯示翻譯中', async ({
  context,
  localServer,
}) => {
  const page = await context.newPage();
  ...
  const state = await evaluate(`({
    ...
    segmentText: document.getElementById('zh-segment').textContent,
  })`);

  expect(state.active).toBe(false);
  expect(state.translateCalled).toBe(0);
  expect(state.statusExists).toBe(false);
  expect(state.segmentText).toBe('這部影片已經有中文字幕');
