...
//
// 結構通則 / 測法:
//   - 25 rawSegments → 3 批 [1, 12, 12](v1.9.19 BATCH=8 → 12 後的切批配置)
//   - Mock sendMessage:batch 0 成功、batch 1 reject、batch 2 成功
//   - 等所有批次都 settle 後檢查:
//       (a) batch 0 + batch 2 的 captionMap entries 都應寫進(總數 = 1 + 12 = 13)
...
test('youtube-batch-allsettled: 後續批次某批 reject 不應拖累其他批的 captionMap 與 batchApiMs', async ({
  ...
  // 25 條 rawSegments → 切成 3 批 [1, 12, 12](v1.9.19 BATCH=12)
  await evaluate(`
    const segs = [];
    for (let i = 0; i < 25; i++) {
      segs.push({
  ...
  // batch 0 (1 條) + batch 2 (12 條) = 13 條應有 captionMap entries
  expect(
    result.captionMapSize,
    `成功的 batch 0+2 共 13 條應寫入 captionMap(實際:${result.captionMapSize})。若 < 13 代表 batch 2 也被 batch 1 reject 拖累。`,
