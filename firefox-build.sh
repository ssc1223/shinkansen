...
# 與 release 上 `shinkansen-firefox-vX.Y.Z-beta.zip` 完全一致的產物。
#
# 用法:
...
#
# 輸出:
#   shinkansen-firefox-<version>-beta.zip(其中 <version> 從 manifest.json 讀出)
#
...
# 驗證重建一致性(reviewer 用):
#   1. ./firefox-build.sh
#   2. unzip -p shinkansen-firefox-<v>-beta.zip manifest.json | jq .
...
rm -rf firefox-build
rm -f "shinkansen-firefox-v${VERSION}-beta.zip"
...
cd firefox-build
zip -r "../shinkansen-firefox-v${VERSION}-beta.zip" .
cd ..
...
echo ""
echo "Done: shinkansen-firefox-v${VERSION}-beta.zip"
echo ""
echo "Verify manifest:"
unzip -p "shinkansen-firefox-v${VERSION}-beta.zip" manifest.json | jq '{version, background, browser_specific_settings}'
