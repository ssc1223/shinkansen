// ─────────────────────────────────────────────────────────────────────────
// readability.js — vendored from @mozilla/readability v0.6.0（Apache-2.0）
// 原始授權見下方 Arc90 / Mozilla 版權標頭，完整條款見 lib/readability.LICENSE。
//
// 用途：「送到 Instapaper」送出前在譯文 DOM 的 clone 上抽乾淨正文，避免整頁
// SPA 噪音讓下游 reader 的 readability 綁架到影片嵌入塊（見 PLAN-send-to-instapaper.md）。
// 以 classic content script 載入（manifest content_scripts），於 isolated world
// 定義全域 Readability；檔尾另掛 window.Readability 供 content-ns.js 取用。
//
// 本地修改（grep "Shinkansen patch" 可定位）:
//   1. REGEXPS.commas 末尾補 U+3001 頓號（中文列舉分隔，上游已含全形逗號 U+FF0C）。
//   2. _cleanConditionally 的逗號計數改用 REGEXPS.commas（含 CJK 變體），避免
//      中文段落因半形逗號少被誤刪。
//   3. 檔尾 window.Readability 全域掛載。
// 兩處評分修正取自姊妹專案 JRead detector 的 seedScore [,，、] CJK 調校。
// ─────────────────────────────────────────────────────────────────────────
/*
...
Readability.prototype = {
  ...
  REGEXPS: {
    ...
    // Shinkansen patch: 末尾 \u3001 為補上的頓號（中文列舉分隔）。上游已含全形
    // 逗號 \uFF0C，但漏了頓號；中文段落常以頓號分隔，計入內容評分後正文評分才
    // 不會偏低（取自 JRead detector 的 seedScore [,，、]）。
    commas: /\u002C|\u060C|\uFE50|\uFE10|\uFE11|\u2E41|\u2E34|\u2E32|\uFF0C|\u3001/g,
  ...
  _cleanConditionally(e, tag) {
    ...
    this._removeNodes(this._getAllNodesWithTag(e, [tag]), function (node) {
      ...
      // Shinkansen patch: 原本只數半形逗號 ","，中文段落多用全形逗號 / 頓號，
      // 會被誤判「逗號太少」而連同正文一起清掉。改用 REGEXPS.commas（含 CJK 變體）
      // 計數，與 _scoreNode 的內容評分一致。
      if (this._getCharCount(node, this.REGEXPS.commas) < 10) {
...
// Shinkansen: expose as isolated-world global for content-script use.
if (typeof window !== "undefined" && !window.Readability) { window.Readability = Readability; }
