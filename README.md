...
- **簡繁互轉完全免費**：翻譯目標是繁體或簡體中文時，相反變體的中文內容走內建 OpenCC 字典本地轉換——不需 API Key、不打任何 API、離線可用，詞組級對照涵蓋台灣慣用詞（軟件→軟體、視頻→影片、內存→記憶體）。工具列圖示選單可開啟「簡繁自動互轉」——勾選當下立即轉換目前頁面、取消勾選立即還原，之後簡體（或繁體）網頁載入時自動轉換，不用手動觸發；混合語言頁面也只有中文段落走免費轉換，其餘照走你選的翻譯引擎
...
- **文件翻譯（PDF / EPUB / TXT / Markdown / HTML）**：上傳檔案整份翻譯——PDF 保留原版面輸出譯文 PDF；EPUB 支援全書術語表（人名譯名跨章節一致）、章節選翻、預覽編輯與雙語對照譯本；TXT / Markdown / HTML 檔沿用同一套章節管線，譯文輸出格式與原檔相同。詳見下方「文件翻譯」段落
...
- **禁用詞清單**：可由你編輯的禁用詞對照表，適用任何翻譯目標語言——多數目標語言預設為空，只有繁體中文（台灣）目標預設帶 25 條（例如視頻 / 軟件 / 數據 / 網絡 / 質量 / 用戶）。會以高顯著性區塊注入到 system prompt 末端，明確要求譯文不可使用左欄詞彙。詳見下方「禁用詞清單」段落
...
## 文件翻譯（PDF / EPUB / TXT / Markdown / HTML）

點工具列 Shinkansen 圖示 → 「翻譯文件」，開啟獨立分頁後把 PDF、EPUB、TXT、Markdown 或 HTML 檔案拖進頁面即可整份翻譯。檔案完全在你的瀏覽器本機解析，除了送給翻譯引擎的文字之外不上傳任何內容。
...
- **上限**：50 頁 / 10 MB；純掃描影像（需 OCR）與加密 PDF 不支援

**EPUB 電子書翻譯**（beta）：

- **全書術語表**：翻譯前掃描全書抽出人名 / 地名 / 術語對照表（含暱稱、簡稱、姓氏單獨出現等變體），過目編輯後全程凍結，確保譯名跨章節一致；可匯出 / 匯入 JSON，也可匯入外部整理的 CSV（兩欄「原文,譯名」，容許表頭列與引號跳脫），系列作續集可重用前作譯名
...
禁用詞清單適用任何翻譯目標語言；多數目標語言預設為空，只有繁體中文（台灣）目標預設帶一份 25 條清單，涵蓋常見的雷區：視頻→影片、音頻→音訊、軟件→軟體、程序→程式、進程→行程（process）、線程→執行緒（thread）、數據→資料、數據庫→資料庫、網絡→網路、信息→資訊、質量→品質、用戶→使用者、默認→預設、創建→建立、實現→實作、運行→執行、發布→發表、屏幕→螢幕、文檔→文件、操作系統→作業系統，另含「劍指→針對」、「痛點→要害」、「硬傷→罩門」這類風格詞，以及兩條留空替換詞的純禁用詞（沒有之一、橫空出世）作為範例。
...
v2.3.2 — 完整功能清單與規格詳見 [SPEC.md](SPEC.md)。
