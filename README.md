...
- **禁用詞清單**（v1.5.6 起）：可由你編輯的禁用詞對照表（預設 25 條，例如視頻 / 軟件 / 數據 / 網絡 / 質量 / 用戶），會以高顯著性區塊注入到 system prompt 末端，明確要求譯文不可使用左欄詞彙。詳見下方「禁用詞清單」段落
...
技術上這份清單會以 `<forbidden_terms_blacklist>` 區塊注入到 system prompt 的最末端，是整段 prompt 中模型注意力最高的位置，也明確標示為「優先級高於任何 stylistic 考量」。若文章本身在討論這些詞彙本身（例如一篇分析「兩岸科技用語差異」的報導），prompt 也指示模型用引號保留原詞作為合理 escape hatch。修改清單後 Shinkansen 會自動讓舊快取失效，不用手動清快取。
...
v1.8.39 — 完整功能清單與規格詳見 [SPEC.md](SPEC.md)。
