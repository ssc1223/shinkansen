...
//   node tools/harness/debug-harness.js
//   TARGET_URL=https://en.wikipedia.org/wiki/Taiwan node tools/harness/debug-harness.js
//   node tools/harness/debug-harness.js --keep              # 不關 browser,留著看
//   node tools/harness/debug-harness.js --no-translate      # 只開頁面+截圖,不觸發翻譯
//   node tools/harness/debug-harness.js --fresh             # 砍 user data dir 重新來
//   SHINKANSEN_HEADED=1 node tools/harness/debug-harness.js # 顯示視窗（除錯用,會搶 focus）
//
...
async function main() {
  ...
  //   SEED_SYNC='{"autoConvertZh":true}' TARGET_URL=... node tools/harness/debug-harness.js --no-translate
  ...
  // SEED_LOCAL:同上但種 chrome.storage.local(apiKey 走 local 不走 sync)。搭配 --fresh
  // 重現「乾淨 profile + 指定模型」的真 API 路徑:
  //   SEED_LOCAL="{\"apiKey\":\"$(cat ~/.shinkansen-test-key)\"}" SEED_SYNC='{"geminiConfig":{"model":"gemini-3.8-flash"}}' node tools/harness/debug-harness.js --fresh
  if (process.env.SEED_LOCAL) {
    const seed = JSON.parse(process.env.SEED_LOCAL);
    await sw.evaluate((s) => new Promise((res) => chrome.storage.local.set(s, res)), seed);
    console.log('[harness] SEED_LOCAL 已種入:', Object.keys(seed).join(', '));
  }
  ...
  //   PROBE_EXPR='window.__SK.STATE.translatedBy' node tools/harness/debug-harness.js ...
