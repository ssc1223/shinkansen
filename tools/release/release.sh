#!/bin/bash
# 用法: ./tools/release/release.sh "改了什麼"
#       SKIP_SAFARI=1 ./tools/release/release.sh "改了什麼"   # 緊急只發 Chrome / Firefox
#       SKIP_TESTS=1  ./tools/release/release.sh "改了什麼"   # 緊急跳過 full test gate(release 後須補跑)
#
...
./tools/release/sync-dist.sh "v${VERSION}" || echo "⚠️  dist 同步失敗,手動補跑:tools/release/sync-dist.sh v${VERSION}"
...
if [ "${SKIP_SAFARI:-0}" != "1" ]; then
  ...
  echo "  macOS Safari (MAS)   : build/shinkansen-macos-v${VERSION}-mas.pkg"
  echo "                         open -a Transporter build/shinkansen-macos-v${VERSION}-mas.pkg"
fi
