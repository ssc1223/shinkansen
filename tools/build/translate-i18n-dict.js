#!/usr/bin/env node
// tools/build/translate-i18n-dict.js — P2 i18n build script
//
...
//
// 用法:
//   node tools/build/translate-i18n-dict.js              # 翻譯兩個 target
//   node tools/build/translate-i18n-dict.js --only zh-CN # 只翻 zh-CN
//   node tools/build/translate-i18n-dict.js --only en
//   node tools/build/translate-i18n-dict.js --dry-run    # 不寫回,只印譯文
//
...
async function buildOne(apiKey, content, target) {
  ...
  const headerComment = target === 'zh-CN'
    ? '  // zh-CN dict — 由 tools/build/translate-i18n-dict.js 從 zh-TW dict 翻譯產出。\n  // 改 zh-TW 後重跑 build script;或人工 review 後直接編輯本段'
    : '  // en dict — 由 tools/build/translate-i18n-dict.js 從 zh-TW dict 翻譯產出';
